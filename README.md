# GIT-DEMO
- 先搜尋Git並下載，下載好之後，找到它並按右鍵，選擇以系統管理員身分執行，一直按NEXT即可完成安裝
...
- 檢視目前有幾個commit
    - git log 
      - 能看到commit的編碼
        - git cat-file -t <commit-sha-1> -> 會顯示 commit
        - git cat-file -p <commit-sha-1> -> 會顯示tree(樹狀)+一段編碼
...
- git branch -M main
- git remote add origin https://github.com/YAWENHSU/git-demo.git (遠端倉庫位址)
